...
class StudentSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        if data["enrollment_year"] > data["graduation_year"]:
            raise serializers.ValidationError(
                "Enrolled year must be before graduated year."
            )
        return data
    ...
    def create(self, validated_data):
        ...
        student = Student.objects.create(
            faculty=faculty, resume=resume, **validated_data
        )
        return student

    def update(self, instance, validated_data):
        instance.name = validated_data.get("name", instance.name)
        instance.enrollment_year = validated_data.get(
            "enrollment_year", instance.enrollment_year
        )
        instance.graduation_year = validated_data.get(
